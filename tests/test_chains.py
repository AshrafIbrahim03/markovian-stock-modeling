...
class TestFreqModelMC(TestCase):
    def test__step(self):
        ...
        chain = FreqModelMC(df=df, train_range=train_range, days=3, att_num=3, sample_range=sample_range, state_func=get_state.get_state_by_perc_change)
        print(chain.cur_state_index)
        print(chain.step(1))
